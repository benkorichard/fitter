...
export default function ActiveWorkout() {
  ...
  function getDoneCountFrom(state, pe) {
    return (state[pe.id] || []).length
  }
  ...
  function getNextIncompleteExerciseIndexFrom(state) {
    if (!plan) return null
    for (let i = 0; i < plan.plan_exercises.length; i++) {
      const pe = plan.plan_exercises[i]
      if (getDoneCountFrom(state, pe) < pe.sets) return i
    }
    return null
  }
  ...
  async function logSet() {
    ...
    const updatedLoggedSets = {
      ...loggedSets,
      [currentPE.id]: [
        ...(loggedSets[currentPE.id] || []),
        { set_number: setNumber, reps_done: reps, weight_used: weight, rpe: rpe, rir: rir, is_warmup: isWarmup },
      ],
    }
    ...
    if (scheme === 'superset') {
      ...
      if (pairPe) {
        const pairDone = getDoneCountFrom(updatedLoggedSets, pairPe)
        const pairNeedsThisRound = pairDone < setNumber && pairDone < pairPe.sets

        if (pairNeedsThisRound) {
          goToExercise(pairIdx, { clearRest: true })
          setRestSeconds(null)
        } else {
          setRestSeconds(plan.rest_time)
          const nextIncomplete = getNextIncompleteExerciseIndexFrom(updatedLoggedSets)
          if (nextIncomplete !== null && nextIncomplete !== currentExIdx) {
            goToExercise(nextIncomplete, { clearRest: false })
          }
        }
      } else {
        setRestSeconds(plan.rest_time)
  ...
  function goToExercise(idx, options = {}) {
    const { clearRest = true } = options
    setCurrentExIdx(idx)
          ...
          {plan.plan_exercises.map((pe, idx) => {
            ...
            return (
              <button
                ...
                onClick={() => goToExercise(idx, { clearRest: false })}
              >
            ...
            {allSetsLogged && (
              <div className="exercise-complete">
                ...
                {currentExIdx < plan.plan_exercises.length - 1 && (
                  <button className="btn-primary" onClick={() => goToExercise(currentExIdx + 1, { clearRest: false })}>
                    Next Exercise →
                  </button>
                )}
              </div>
